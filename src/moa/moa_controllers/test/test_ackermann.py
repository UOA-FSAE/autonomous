...
from ackermann_to_can import compress_floats, ackermann_to_can_parser, ack_to_can
...
def test_ackermann_to_can_parser_out_of_bounds(node):
    ack_msg = AckermannDriveStamped()
    ack_msg.drive.speed = 130/3.6
    assert ackermann_to_can_parser(node, ack_msg) is None
...
def test_ackermann_to_can_parser_valid_values(node):
    ack_msg = AckermannDriveStamped()
    ...
    can_msg = ack_to_can().ackermann_to_can_parser(node, ack_msg)
    assert can_msg == expected_result

def test_ack_to_can_publish_callback(node):
