...
if __name__ == "__main__":
    # --- Tight sine wave with rapid chicanes ---
    x_vals = np.linspace(0, 400, 800)  # compressed x-range to make curves tight
    y_vals = 100 * np.sin(x_vals * 2 * np.pi / 100)  # short wavelength (tight left-right turns)

    centre_pts = list(zip(x_vals, y_vals))

    # --- Run the segmentation pipeline ---
    fits = segment_centerline_and_fit_arcs(centre_pts, window_length=30, step_size=10)
    fits = classify_arcs_vs_straights(
        fits,
        max_rms_err=0.5,
        min_arc_angle=0.2,
        max_R_for_corner=300  # lower this to detect tighter turns as corners
    )
    segs = consolidate_segments(fits, min_straight_len=15)  # keep small straights as is!
